...
general_software(){
	...
	arch-chroot /mnt /bin/bash -c  "pacman -S --noconfirm ttf-dejavu wqy-bitmapfont wqy-microhei wqy-zenhei noto-fonts"
	...
	arch-chroot /mnt /bin/bash -c  "yay -S --noconfirm ttf-wps-fonts wps-office-mui-zh-cn wps-office-mime-cn wps-office-cn"
...
case $desktop in
	...
	arch-chroot /mnt /bin/bash -c  "pacman -S --noconfirm git"
	arch-chroot /mnt /bin/bash -c "sudo git clone https://github.com/ayamir/dotfiles.git /"
	arch-chroot /mnt /bin/bash -c  "pacman -S --noconfirm nerd-fonts-ibm-plex-mono nerd-fonts-fira-code nerd-fonts-jetbrains-mono ttf-sarasa-gothic
	  dmenu rofi alacritty kitty st wezterm neovim glrnvim neovide doom emacs vscode sublime-text-nightly picom-jonaburg-git fish zathura
	   mpd ncmpcpp rofi dunst xob i3lock-color xautolock conky xsettingsd libinput libinput-gestures xdotool"
	echo "fcitx5 &" >> /mnt/root/.xinitrc
	echo 'exec dwm' >> /mnt/root/.xinitrc
	;;
	3)
