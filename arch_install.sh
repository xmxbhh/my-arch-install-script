...
user_passwd(){
	...
	if [[ $judge =~ ^(yes|y|Y| ) ]]; then
		read -r -p "Please enter your user name:" user_name
		arch-chroot /mnt /bin/bash -c "useradd -m -g users -G wheel -s /bin/bash ${user_name}"
		read -r -p "Please enter your $user password:" user_passwd
		arch-chroot /mnt /bin/bash -c "echo ${user_name}:${user_passwd} | chpasswd"
	else
		echo "good"
	fi
}
...
xianka(){
	    ...
	    if ! [[ -z ${pci_intel} ]] ; then
	        arch-chroot /mnt /bin/bash -c  "pacman -S --noconfirm install-ucode"
...
echo  "|| echo -e     2 dwm: Personal modified version ||"
